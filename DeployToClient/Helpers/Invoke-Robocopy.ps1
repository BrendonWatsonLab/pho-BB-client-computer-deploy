...
.Parameter PassThru
    ...
    EncounteredError - Boolean indicating whether an error occurred or a normal successful operation was performed.
...
function Invoke-Robocopy
{
    ...
    Process
    {
        foreach ($item in $Destination) {
            ...
            $Result | Add-Member -NotePropertyName EncounteredError -NotePropertyValue ($Result.ExitCode -band [Robocopy.ExitCode]'FailedCopyAttempts, FatalError')
            # $Result.EncounteredError = ($Result.ExitCode -band [Robocopy.ExitCode]'FailedCopyAttempts, FatalError')
